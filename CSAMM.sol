...
contract CSAMM {
    ...
    function swap(address _tokenIn, uint _amountIn) external returns (uint amountOut) {
        ...
        uint amountIn = tokenIn.balanceOf(address(this)) - resIn; // safety reasons

        amountOut = amountIn * 997 / 1000;

        (uint res0, uint res1) = isToken0 ? (resIn + _amountIn, resOut - amountOut) : (resOut - _amountIn, resIn + amountIn);
        _update(res0, res1);
